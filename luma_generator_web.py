...
for j in range(3):
r += b[p:p+6]; p += 6
r += float_to_hex(vals[i][j2])
r += b[p+8:p+10]; p += 10
r += float_to_hex(vals[i][j2+1])
p += 8
r += b[p:]
out.append(r)
return "".join(out)
...
with st.expander("Parser ID14", False):
h = st.text_area("HEX", SHARP_ID14_DEFAULT_HEX, height=200)
if st.button("Parse"):
st.session_state.vals = parse_id14(h)
st.rerun()
vals = []
for i in range(5):
with st.expander(f"Level {i}", True):
c = st.columns(3)
