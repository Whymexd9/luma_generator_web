...
def generate_id14(vals):
    ...
    for i in range(5):
        for j in range(3):
            p = src.find("0a140d", p)
            out.append(src[p:p+6])
            p += 6
            out.append(f2h(vals[i][j*2]))
            p = src.find("1d", p)
            out.append("1d")
            p += 2
            out.append(f2h(vals[i][j*2+1]))
...
for i, name in enumerate(levels):
    with st.expander(name, True):
        c = st.columns(3)
        l1  = c[0].number_input("L1",  value=st.session_state.id14_vals[i][0], format="%.4f", key=f"l1_{i}")
        l1a = c[1].number_input("L1A", value=st.session_state.id14_vals[i][1], format="%.4f", key=f"l1a_{i}")
        l2  = c[0].number_input("L2",  value=st.session_state.id14_vals[i][2], format="%.4f", key=f"l2_{i}")
        l2a = c[1].number_input("L2A", value=st.session_state.id14_vals[i][3], format="%.4f", key=f"l2a_{i}")
        l3  = c[0].number_input("L3",  value=st.session_state.id14_vals[i][4], format="%.4f", key=f"l3_{i}")
        l3a = c[1].number_input("L3A", value=st.session_state.id14_vals[i][5], format="%.4f", key=f"l3a_{i}")
        vals.append([l1, l1a, l2, l2a, l3, l3a])

if st.button("Generate HEX"):
    hexout = generate_id14(vals)
    st.code(hexout)
    st.text_input("Patch", value=f"0x{SHARP_ID14_ADDR:X}:{hexout}")

with st.expander("Parse HEX"):
    h = st.text_area("HEX", value=SHARP_ID14_DEFAULT, height=200)
    if st.button("Parse"):
        parsed = parse_id14(norm(h))
        for i in range(5):
            st.session_state.id14_vals[i] = parsed[i]
        st.rerun()
